...
export default function RedistributionQueue({ alert }: { alert: RiskAlert | null }) {
  ...
  async function approve(c: RedistributionCandidate) {
    ...
    setApprovedIds((prev) => new Set(prev).add(c.donor_facility_id));
  }
  ...
  return (
    <div className="bg-white border border-[#D5DEDC] rounded-xl shadow-sm p-5">
      ...
      <p className="text-xs text-[#4C6567] mb-3">
        Ranked by urgency, travel time, and donor shortfall risk. Recommend-only — a person approves every transfer.
      </p>
      ...
      <div className="space-y-2">
        {candidates.map((c) => (
          ...
          >
            ...
            {canApprove ? (
              approvedIds.has(c.donor_facility_id) ? (
                <span className="text-xs font-semibold text-[#2E8B57]">Approved</span>
              ) : (
                <button
                  onClick={() => approve(c)}
